...
volatile uint8_t kbd_bit = 0;
volatile uint8_t kbd_data=0;
uint8_t kbd_buffer[16];
volatile uint8_t kbd_ptr=0;
...
void video_putch(uint8_t ch) {

    if(ch!=8) {
        vram[cursor_x + cursor_y * NTSC_X_CHARS] = ch;
        cursor_x++;
        if (cursor_x >= NTSC_X_CHARS) {
            cursor_x = 0;
            cursor_y++;
...
void TIM1_CC_IRQHandler(void) {
    ...
    if ((ntsc_line >= NTSC_SCAN_START - 1)
        ...
        char_y = (ntsc_line + 1 - NTSC_SCAN_START) / 16;
        slice_y = (ntsc_line + 1 - NTSC_SCAN_START) % 16;
        slice_y >>= 1;
